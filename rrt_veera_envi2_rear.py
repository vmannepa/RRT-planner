...
"""This code mainly implements RRT motion planning technique using pygame animation 
   ...
   NON-HOLONOMIC : REAR WHEEL CONSTRIANTS""" 
...
pygame.display.set_caption('RRT NONHOLONOMIC RUNGEKUTTA REAR WHEEL CONSTRAINTS ')
black = (  0,   0,   0)
white = (255, 255, 255)
red   = (255,   0,   0)
blue  = (0,   0,   255)
...
fps = 60
screen.fill(white)
pygame.draw.rect(screen, black, [300, 0, 100, 350],1)
pygame.draw.rect(screen, black, [300, 450, 100, 270],1)
pygame.draw.rect(screen, black, [525, 300, 50, 200],1)
pygame.draw.rect(screen, black, [700, 0, 100, 350],1)
pygame.draw.rect(screen, black, [700, 450, 100, 270],1)
...
def runge_k_4(p1,steer):
    ...
    for i in range(n):
	p=curve_list[-1]
        k1 = h*cos(p[2]),h*sin(p[2]),0.067*tan(steer)
	k2 = 0.5*k1[0]+h*cos(p[2]),0.5*k1[1]+h*sin(p[2]),0.5*k1[2]+0.067*tan(steer)
	k3 = 0.5*k2[0]+h*cos(p[2]),0.5*k2[1]+h*sin(p[2]),0.5*k2[2]+0.067*tan(steer)
	k4 = 0.5*k3[0]+h*cos(p[2]),0.5*k3[1]+h*sin(p[2]),0.5*k3[2]+0.067*tan(steer)
...
def main():
    ...
    obstacle1= Block(blue,100,350)
    obstacle1.set_position(300,0)
    ...
    obstacle2= Block(blue,100,270)
    obstacle2.set_position(300,450)
    ...
    obstacle3= Block(blue,50,200)
    obstacle3.set_position(525,300)
    ...
    obstacle4= Block(blue,100,350)
    obstacle4.set_position(700,0)
    ...
    obstacle5= Block(blue,100,270)
    obstacle5.set_position(700,450)
